...
| `/storage/music`, `/storage/media` (music library; `media/` holds the TV + movie libraries and download landing area) | Large and *re-acquirable* — members keep originals; music is re-rippable/re-downloadable; recordings can be re-captured. Sending TBs to metered cloud buys no durability. Flip `BACKUP_EXCLUDES` to include them. |
...
| 2026-07-17 | **PASS — coverage gap closed, photo restore proven. Gate met.** Initial seed uploaded both streams to B2: `data` snapshot `bd82a8a3` (2.581 GiB — dumps/configs/secrets) + `storage` snapshot `1187b2c9` (242.800 GiB / 103,917 files, 16h57m). `restic check` clean ("no errors were found", 14,881 packs / 104 indexes). Per-stream retention applied correctly (data 7/4/6, storage 7/4/0). **Blob restore test:** a small MinIO-inlined object *and* a real ~1.7 GB multipart photo/video (87 parts) each restored from B2 **byte-identical** to the live MinIO original (recursive sha256 match). This is the first snapshot ever to contain the photo blobs, and the first proven photo restore. |
