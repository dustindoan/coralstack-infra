...
Configure five env vars via a LaunchAgent so the Ollama Mac app picks them up at every login:

- `OLLAMA_HOST=0.0.0.0` — bind to all interfaces (default is loopback-only, unreachable from the Apps VM)
...
- ~~**Pin Ente image versions.**~~ ✅ Done 2026-07-17: both images are digest-pinned in `services/ente/docker-compose.yml` (Ente ships only a rolling `:latest`, so a digest is the pin), with Renovate opening bump PRs. See [docs/APP_UPDATES.md](APP_UPDATES.md).
