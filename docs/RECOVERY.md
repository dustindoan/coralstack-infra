...
| `OLLAMA_HOST=0.0.0.0`, `OLLAMA_FLASH_ATTENTION=1`, `OLLAMA_KV_CACHE_TYPE=q8_0`, `OLLAMA_KEEP_ALIVE=-1` | `~/Library/LaunchAgents/com.ollama.host.{sh,plist}` (see [PROXMOX_MIGRATION.md](PROXMOX_MIGRATION.md) Phase 4c step 4) | Network binding + 8-bit KV cache + keep model loaded indefinitely (no 5min idle reload). LaunchAgent persists across reboots and includes a self-heal block in case the Ollama Login Item wins the boot race |
