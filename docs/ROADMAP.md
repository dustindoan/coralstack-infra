...
| Uptime Kuma config-as-code | ✅ **viable — [AutoKuma](https://github.com/bigboot/autokuma) v2.0.0 verified against Kuma 2.4.0** (spike 2026-08-25: file-only mode created both an HTTP *and* a push monitor on a throwaway 2.4.0 instance) | Kuma's REST API is read-only, all writes go over an internal unversioned Socket.IO protocol, and **v2 removed the JSON export/import** ([migration notes](https://github.com/louislam/uptime-kuma/wiki/Migration-From-v1-To-v2)) — the data directory is the only supported backup, which is why [backup.sh](../services/backup/backup.sh) now dumps `kuma.db` explicitly. Given that, third-party tooling is the only config-as-code route. **AutoKuma** (882★, MIT, active) reconciles monitors from Docker labels *or* static `.json`/`.toml` files; **file-only mode (`AUTOKUMA__DOCKER__ENABLED=false`) is preferred here** — it keeps the declaration in one reviewable file and avoids a second holder of `docker.sock` (Portainer is the deliberate exception, see [ADMIN_ACCESS](ADMIN_ACCESS.md)). Alternative: [breml/terraform-provider-uptimekuma](https://github.com/breml/terraform-provider-uptimekuma) (65★, v0.4.0 Jul 2026), which would fit the Terraform followups already listed in [PROXMOX_MIGRATION](PROXMOX_MIGRATION.md). **Rejected:** the Python wrappers — mainstream [lucasheld/uptime-kuma-api](https://github.com/lucasheld/uptime-kuma-api) is 1.21–1.23 only, and the sole 2.x fork targets 2.0.0-beta.2 against our 2.4.0. Note **none of these can bootstrap the admin account** (all need existing credentials), so first-run claiming stays manual — which is also [SEC-2](SECURITY_PASS.md#sec-2-medium--unclaimed-first-run-setup-wizard-on-a-public-vhost). Manual setup is not wasted: AutoKuma reconciles and can adopt alongside hand-made monitors |
