...
| Uptime Kuma (monitoring + public `status.` page) | ✅ deployed; monitors declared as code and an ntfy channel wired to all of them (verified 2026-09-04) — **the open gap is the external dead-NUC check**, [MONITORING.md §4](MONITORING.md) | [MONITORING.md](MONITORING.md) — independent status page mitigates the PWA failure-state trap; hosts the backup + SMART dead-man's-switches. Config lives only in Kuma's SQLite DB (no config-as-code, same limitation that removed Homarr), so the monitor set is a UI checklist |
...
| Uptime Kuma config-as-code | ✅ **built** — [services/autokuma](../services/autokuma/) | The monitor set is declarative `.toml` in the repo; AutoKuma 2.0.0 reconciles Kuma against it, so monitors survive a rebuild instead of being re-clicked. File-only mode (no `docker.sock`). Push tokens are **pinned**, so `setup.sh` generates them and wires `HEALTHCHECK_URL` at both ends — no copy-paste from the UI. Verified against Kuma 2.4.0: all 10 definitions created, zero warnings, pinned push URL accepted a live heartbeat. **Gotcha encoded in the compose file:** AutoKuma's `/data` must persist or every container recreation duplicates every monitor. Still manual: the notification channel (AutoKuma's support for it is flagged experimental) and Kuma's first-run admin account (nothing can bootstrap it — see [SEC-2](SECURITY_PASS.md#sec-2-medium--unclaimed-first-run-setup-wizard-on-a-public-vhost)). **Rejected:** the Python wrappers (1.21–1.23 only; the 2.x fork targets 2.0.0-beta.2). Also note v2 removed Kuma's JSON export/import entirely |
